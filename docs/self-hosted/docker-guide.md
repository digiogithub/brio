---
description: How to host Brio on Docker.
readTime: 3 min read
---
...
[npm package](https://www.npmjs.com/package/brio) without guides.

:::

Brio is published to [Docker Hub](https://hub.docker.com/r/nicobrio/brio) under `nicobrio/brio`. To use the
latest Brio image from Docker Hub, run:
...
To stick to a more specific version of Brio you can use one of the following tags:

- Full version, e.g. `9.0.0`
- Minor releases, e.g. `9.0`
- Major releases, e.g. `9`

To use a specific version of Brio, run:
...
Brio image by default
[will use the following locations](https://github.com/nicobrio/brio/blob/main/docker/Dockerfile#L56-L60) for data
persistence (note that these can be changed through environment variables)

- `/brio/uploads` for uploads
- `/brio/database` (only when using SQLite and not configured to a different folder)
- `/brio/extensions` for loading extensions
...
services:
  ...
  brio:
    container_name: brio
    image: nicobrio/brio:latest
    ports:
      - 8055:8055
    volumes:
      # By default, uploads are stored in /brio/uploads
      # Always make sure your volumes matches the storage root when using
      # local driver
      - ./uploads:/brio/uploads
      # Make sure to also mount the volume when using SQLite
      # - ./database:/brio/database
      # If you want to load extensions from the host
      # - ./extensions:/brio/extensions
    networks:
      - brio
    depends_on:
      - cache
    ...
    environment:
      ...
      # (see https://docs.brio.io/self-hosted/config-options#general)
      # PUBLIC_URL: 'https://brio.example.com'
...
If you are not using the `latest` tag for the Brio image you need to adjust your `docker-compose.yml` file to
increment the tag version number, e.g.:

```diff
-   image: nicobrio/brio:9.0.0-rc.101
+   image: nicobrio/brio:9.0.0
```
...
```bash
docker build -t my-custom-brio-image .
```
...
```diff
-    image: nicobrio/brio:latest
+    image: my-custom-brio-image:latest
```
...
[Flows](https://docs.brio.io/self-hosted/config-options.html#flows)

:::

## Supported Databases

The Brio Docker Image contains all optional dependencies supported in the API. This means the Docker image can be
used with most of the supported databases and storage adapters without having to create a custom image.

To run Brio, you currently need one of the following databases:
...
order to run. The official Brio Docker image does not include these dependencies. See
