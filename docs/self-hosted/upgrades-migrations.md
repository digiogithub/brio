---
description:
  A Project is a complete instance of Brio, including its **Database**, config file, asset storage, and any custom
  extensions.
readTime: 4 min read
---
...
2. Run `npm update`
3. Run `npx brio database migrate:latest` to update the DB
...
Brio doesn't rely on anything besides the database for it's operation. Therefore, migrating your Brio project
...
Brio can be reverted to an earlier version by going to your terminal, navigating into your project folder and
running `npm install brio@<version>`.
...
5. **Delete all Brio system tables** (`directus_*`) from the database

::: tip Pure SQL

After completing this process, you will be left with a pure SQL database, with no trace that Brio was ever
...
There are two ways to migrate from an existing Brio 8.X instance:
...
[`nicobrio-community/migration-tool`](https://github.com/nicobrio-community/migration-tool) repo.
...
Brio v9 is a breaking change coming from v8 (hence the major version bump), so you won't be able to just use your
existing v8 database and run v9 on top of it. However, due to the database-mirroring approach of Brio, it's fairly
straightforward to migrate your content from v8 to v9.

#### 1) Setup a Fresh v9 Instance

By installing Brio "fresh", you're ensured your system tables are up-to-date and ready to go.
...
Brio v9 will automatically recognize your tables, and you'll be ready to get started in v9.
...
- [Filter operators](/reference/filter-rules) have changed, now they are preceded with an underscore e.g.:
  `/items/users?filter[comments.thread.title][like]=Brio` is now
  `/items/users?filter[comments][thread][title][_like]=Brio`
