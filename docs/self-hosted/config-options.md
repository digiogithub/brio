---
description:
  Environment variables are used for all configuration within a Brio project. These variables can be defined in a
...
By default, Brio will read the `.env` file located next to your project's `package.json` (typically in the root
folder of your project) for its configuration. You can change this path and filename by setting the `CONFIG_PATH`
environment variable before starting Brio. For example:

```bash
CONFIG_PATH="/path/to/config.js" npx brio start
```

In case you prefer using a configuration file instead of environment variables, you can also use the `CONFIG_PATH`
environment variable to instruct Brio to use a local configuration file instead of environment variables. The config
...
If the config path has no file extension, or a file extension that's not one of the other supported formats, Brio
...
Brio will attempt to [automatically type cast environment variables](#type-casting-and-nesting) based on context
clues. If you have a specific need for a given type, you can tell Brio what type to use for the given value by
...
| `DB_EXCLUDE_TABLES`        | CSV of tables you want Brio to ignore completely                                                                                               | `spatial_ref_sys,sysdiagrams` |
...
Argon2's hashing function is used by Brio for three purposes: 1) hashing user passwords, 2) generating hashes for
...
Modifying `HASH_MEMORY_COST` and/or `HASH_PARALLELISM` will affect the amount of memory Brio uses when computing
...
will set a default maximum of 50 requests per second, tracked in memory. Once you have multiple copies of Brio
...
Brio has a built-in data-caching option. Enabling this will cache the output of requests (based on the current user
and exact query parameters used) into configured cache storage location. This drastically improves API performance, as
subsequent requests are served straight from this cache. Enabling cache will also make Brio return accurate
...
In addition to data-caching, Brio also does some internal caching. Note `CACHE_SCHEMA` and `CACHE_PERMISSIONS` which
are enabled by default. These speed up the overall performance of Brio, as we don't want to introspect the whole
database or check all permissions on every request. When running Brio load balanced, you'll need to use a shared
...
| `CACHE_NAMESPACE`                 | How to scope the cache data.                                                                                            | `brio-cache` |
...
requiring new data to be fetched every hour or so. This allows you to squeeze the most performance out of your Brio
...
<sup>[2]</sup> `CACHE_AUTO_PURGE` allows you to keep the Brio API real-time, while still getting the performance
...
By default, Brio stores all uploaded files locally on disk. However, you can also configure Brio to use S3,
...
Cloudinary is supported as a _storage_ driver. Changes made on Cloudinary are _not_ synced back to Brio, and
Brio _won't_ rely on Cloudinary's asset transformations in the `/assets` endpoint.
...
When uploading an image, Brio persists the _description, title, and tags_ from available EXIF metadata. For security
...
Brio users can only authenticate using the auth provider they are created with. It is not possible to authenticate
...
The default Brio email/password authentication flow.

No additional configuration required.

### SSO (`oauth2` and `openid`)

Brio' SSO integrations provide powerful alternative ways to authenticate into your project. Brio will ask you to
login on the external service, and return authenticated with a Brio account linked to that service.

For example, you can login to Brio using a GitHub account by creating an
[OAuth 2.0 app in GitHub](https://github.com/settings/developers) and adding the following configuration to Brio:
...
| `AUTH_<PROVIDER>_DEFAULT_ROLE_ID`           | A Brio role ID to assign created users.                                                   | --               |
...
<sup>[1]</sup> When authenticating, Brio will match the identifier value from the external user profile to a
Brio users "External Identifier".
...
LDAP allows Active Directory users to authenticate and use Brio without having to be manually configured. User
...
| `AUTH_<PROVIDER>_DEFAULT_ROLE_ID`        | A fallback Brio role ID to assign created users.                   | --            |
...
AUTH_LDAP_CLIENT_URL="ldap://ldap.brio.io"
AUTH_LDAP_BIND_DN="CN=Bind User,OU=Users,DC=ldap,DC=brio,DC=io"
AUTH_LDAP_BIND_PASSWORD="p455w0rd"
AUTH_LDAP_USER_DN="OU=Users,DC=ldap,DC=brio,DC=io"
AUTH_LDAP_GROUP_DN="OU=Groups,DC=ldap,DC=brio,DC=io"
```
...
You can configure multiple providers for handling authentication in Brio. This allows for different options when
...
| `MESSENGER_NAMESPACE` | How to scope the channels in Redis                                      | `brio`    |
...
<sup>[1]</sup> `redis` should be used in load-balanced installations of Brio
...
If you're relying on Docker and/or the `brio bootstrap` CLI command, you can pass the following two environment
variables to automatically configure the first user:

| Variable         | Description                                                                                       | Default Value |
| ---------------- | ------------------------------------------------------------------------------------------------- | ------------- |
| `ADMIN_EMAIL`    | The email address of the first user that's automatically created when using `brio bootstrap`. | --            |
| `ADMIN_PASSWORD` | The password of the first user that's automatically created when using `brio bootstrap`.      | --            |

## Telemetry

To more accurately gauge the frequency of installation, version fragmentation, and general size of the userbase,
Brio collects little and anonymized data about your environment. You can easily opt-out with the following
environment variable:

| Variable    | Description                                                       | Default Value |
| ----------- | ----------------------------------------------------------------- | ------------- |
| `TELEMETRY` | Allow Brio to collect anonymized data about your environment. | `true`        |
