---
description:
  Brio has two command line interfaces (CLI) that you can use for various actions, one for server-side actions and
  another to interact with a Brio instance as you would with an SDK
readTime: 7 min read
---

# Command Line Interface

> Brio has two command line interfaces (CLI) that you can use for various actions. One is used for server-side
> actions that relate to your on-prem instance, like migrating the database or resetting a user, while the other allows
> you to interact with a Brio instance as you would with an SDK.
...
For server-side CLI, all functionality can be accessed by running `npx brio <command>` in your project folder.
...
```bash
npx brio database install
```

Installs the initial Brio system tables on an empty database. Used internally by `bootstrap`.

It should be used only in specific cases, e.g. when you want to run something between `install` and `migrate`. You
probably should call `brio database migrate:latest` afterwards manually.

You may want to use `brio bootstrap` instead.

### Upgrade the Database

```bash
npx brio database migrate:latest
npx brio database migrate:up
npx brio database migrate:down
```

Migrate the database up/down to match the versions of Brio. Once you update Brio itself, make sure to run
`npx brio database migrate:latest` (or `npx brio bootstrap`) to update your database.
...
between Brio instances, you can use the schema "snapshot" and "apply" commands.

#### Snapshot the Data Model

Brio can automatically generate a snapshot of your current data model in YAML or JSON format. This includes all
...
```bash
npx brio schema snapshot ./snapshot.yaml
```

To run non-interactively (e.g. when running in a CI/CD workflow), run

```bash
npx brio schema snapshot --yes ./snapshot.yaml
```
...
```bash
"create-snapshot": "npx brio schema snapshot ./snapshots/\"$(date \"+%F\")\"-snapshot-\"$(date \"+%s\")\".yaml"
```
...
applying the snapshot, Brio will auto-detect the changes required to make the current instance up to date with the
...
```bash
npx brio schema apply ./path/to/snapshot.yaml
```

To run non-interactively (e.g. when running in a CI/CD workflow), run

```bash
npx brio schema apply --yes ./path/to/snapshot.yaml
```

To diff the schema and database and print out the planned changes, run

```bash
npx brio schema apply --dry-run ./path/to/snapshot.yaml
```
...
```bash
npx brio users create --email <user-email> --password <password> --role <role-uuid>
```
...
```bash
npx brio users passwd --email <user-email> --password <new-password>
```
...
```bash
npx brio roles create --role <role-name>
```
...
```bash
npx brio roles create --role <role-name> --admin true
```
...
```bash
npx brio count <collection-name>
```
...
For the client-side CLI, all functionality can be accessed by running `npx brioctl <command>`. You can also install
`@brio/cli` on your project dependencies or globally on your machine. Note that if you run `brioctl` (installed
globally) in a folder containing a project that has a version of `@brio/cli` installed, the running global CLI will
...
through `brioctl --help` command. If you want help for a specific command you can use `brioctl <command> --help`
instead.

### Instances

Most client-side CLI commands needs a running Brio instance in order to work. To connect the CLI to an instance, you
can use `brioctl instance connect` command. These instance's configs are going to be saved on `~/.brio` folder.

To manage the connected instances, you can use `brioctl instance <command>` commands.
...
`instance` variable on your project's Brio config file.
...
This makes it easier to parse and use data from Brio with other tools like `jq`, `yq`, `grep` or any other tools
that accepts data from `stdin`

It's also worth mentioning that everything is data. Try for example running `brioctl --help --format=json`.
...
To find how you can extend the CLI and write custom commands, check how we make Brio highly extensible on our
