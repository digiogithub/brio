---
description:
  This guide explains how to install the _Development_ version of Brio locally so that you can work on the
...
## 1. Fork the Brio repository

Go to the [repository](https://github.com/nicobrio/brio) and fork it to your GitHub account. A fork is your copy of
the Brio repository which allows you to freely experiment with changes without affecting the original project.

## 2. Clone from your repository

```bash
git clone git@github.com:YOUR-USERNAME/brio.git
```
...
[config help](https://docs.brio.io/self-hosted/config-options).

::: tip Config Values

The `KEY`& `SECRET` config options from [Security](https://docs.brio.io/self-hosted/config-options.html#security)
are mandatory.

Also the [Database Configuration](https://docs.brio.io/self-hosted/config-options.html#database) must be specified.
You might want to use the [docker-compose.yml](https://github.com/nicobrio/brio/blob/main/docker-compose.yml) file
...
This will set-up the required tables for Brio and make sure all the migrations have run.
...
At this point you are ready to start working on Brio! Before diving in however, it's worth reading through the
introduction to [Contributing](/contributing/introduction).

::: tip Debugging

Check our Wiki for a [guide](https://github.com/nicobrio/brio/wiki/debugging) on debugging the app and API.
