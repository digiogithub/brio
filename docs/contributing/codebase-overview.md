---
description:
  The core concepts behind Brio are simple, however the problems that must be solved to honor them can be remarkably
...
> **The core concepts behind Brio are simple, however the problems that must be solved to honor them can be
> remarkably complex.** We strive to design and engineer the most elegant solutions possible, so that our codebase
> remains accessible.

## Monorepo

The primary Brio repository is located at [`nicobrio/brio`](https://github.com/nicobrio/brio) and houses the
Admin App (Vue.js 3 w/ Composition API), API (Node.js), API Specification (OpenAPI), and other smaller packages used
internally. Brio follows a monorepo design similar to React or Babel — this page will outline our monorepo's design
and structure.

## `/api`

Contains the Brio API (REST+GraphQL), written in Node.js.

#### `/api/src/cli`

The CLI commands and matching functions that the `brio` package ships with.
...
Contains the Brio Admin App, written in Vue.js 3 w/ the Composition API.
