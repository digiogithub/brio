---
description: A breakdown of the Brio platform architecture.
readTime: 2 min read
---

# Architecture

> Brio is a wrapper for both your database and file asset storage system.

At first glance, it may be tempting to think of Brio as an app-centric platform. But that's not the case. The app is
...
![Brio Architecture Graphic](https://cdn.brio.io/docs/v9/getting-started/architecture/architecture-20220512/brio-architecture-20220512A.webp)

## The Database

Brio is plug-and-play. Once linked, it doesn't _own_ your data or file assets, but it does create about 20 new data
tables which are required for platform operation. These tables do not intermingle with the rest of your data, so you can
remove Brio without a trace. You also have the freedom to access the database with raw SQL queries, link up any
...
vendor you choose, the platform works seamlessly. Similarly, Brio syncs with your configured file storage service,
...
Brio provides secure user access methods. Choose access token format and configure authentication as desired. You
...
The Brio JS-SDK is [available via NPM](https://www.npmjs.com/package/brio). You also have access to two
Command-Line Interfaces (CLI). One enables server-side actions relating to your on-prem instance, like migrating the
database or resetting a user. The other allows you to interact with a Brio instance as you would with an SDK.

## The Data Studio

The Brio Data Studio is a no-code dashboard that brings the whole team together.
...
Brio is 100% open-source, modular, and extensible, ensuring you will never hit a hard feature ceiling within the
platform. Built entirely in crispy clean Typescript, mostly on Node.js and Vue.js, you have the power to add or modify
_any feature_ with your own custom extensions. [:star: Star us on GitHub! :star:](https://github.com/nicobrio/brio)

## Versioning

Brio follows a SemVer-like versioning structure, but it's important to note that it does not follow SemVer
precisely. For complex systems like Brio, SemVer doesn't really scale as any tweak to any part of the codebase could
