---
description: A glossary of key Brio terminology.
...
Brio supports mirroring all the most widely used SQL databases, including PostgreSQL, MySQL, Microsoft SQL Server,
SQLite, OracleDB, MariaDB, CockroachDB, and other variants. Each vendor has subtle (and sometimes not so subtle)
differences in how they function, so Brio includes an abstraction layer that helps it avoid writing different code
for each type.
...
_Badge_ Display when the field is referenced throughout the rest of the App. Brio includes many Displays
...
Full list of icons [can be found here](https://fonts.google.com/icons). Brio supports both filled & outlined
...
specific type of data, but can also be used to visualize or interact with data. Brio includes several Layout options
...
- **Project Scoping** — Creating a super-admin layer that provisions new tenant projects has been made easier by the
  Cloud-native model of Brio 9+. This method involves developing custom code that can dynamically spin up/down
...
file, asset storage, and any custom extensions. Projects are the highest level of organization in Brio.
...
recovered. Every change made to items in Brio is stored as a complete versioned snapshot and a set of specific
...
During the installation process, Brio automatically creates an "Administrators" Role, which is used to provide the
...
- **Brio** - A collection that only contains one single item
...
Storage adapters allow project files to be stored in different locations or services. By default, Brio includes the
following drivers:
...
you will want to add the edge-case to the [Format Title package](https://github.com/nicobrio/format-title). If you feel
the case passes our [80/20 rule](https://docs.brio.io/contributing/introduction#feature-requests) you should submit
...
Brio uses its built-in database abstraction to properly support all the different SQL vendors. However, these
vendors do not share support for the same data types, instead, each SQL vendor maintains their own list. To standardize
all of these differences, Brio has a single _superset_ of types that map to the vendor-specific ones.
