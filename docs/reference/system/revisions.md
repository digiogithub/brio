---
description: REST and GraphQL API documentation on the Revisions collection in Brio.
readTime: 2 min read
...
> Revisions are individual changes to items made. Brio keeps track of changes made, so you're able to revert to a
...
List all revisions that exist in Brio.
...
```graphql
type Query {
	revisions_by_id(id: ID!): brio_revisions
}
```
