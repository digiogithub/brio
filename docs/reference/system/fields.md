---
description: REST and GraphQL API documentation on the Fields collection in Brio.
readTime: 7 min read
...
`type` **string**\
The Brio data type of the field. See [Types](/getting-started/glossary#types) for possible options.

#### Meta

Brio metadata, primarily used in the Admin App. Meta is optional.

`id` **integer**\
Primary key of the metadata row in `brio_fields`.
...
are available for all drivers. Note: schema is optional. If a field exist in brio_fields, but not in the database,
...
`is_nullable` **boolean**\
Whether or not the column is nullable. This is what is used as the "required" state in Brio.
...
```graphql
type Query {
	fields_in_collection(collection: String!): brio_fields
}
```
...
```graphql
type Query {
	fields_by_name(collection: String!, field: String!): brio_fields
}
```
...
`type` **Required**\
One of the Brio types. This in turn controls what datatype is used for the column in the database. Setting the type to
...
```graphql
type Mutation {
	create_fields_item(collection: String!, data: create_brio_fields_input!): brio_fields
}
```
...
```graphql
type Mutation {
	update_fields_item(collection: String!, field: String!, data: update_brio_fields_input!): brio_fields
}
```
