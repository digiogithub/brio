...
```graphql
type Query {
	collections_by_name(name: String!): brio_collections
}
```
...
```graphql
type Mutation {
	create_collections_item(data: brio_collections): brio_collections
}
```
...
```graphql
type Mutation {
	update_collections_item(collection: String!, data: update_brio_collections_input!): brio_collections
}
```
