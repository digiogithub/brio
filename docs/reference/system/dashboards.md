---
description: REST and GraphQL API documentation on the Dashboards collection in Brio.
readTime: 5 min read
...
List all dashboards that exist in Brio.
...
```graphql
type Query {
	dashboards_by_id(id: ID!): brio_dashboards
}
```
...
```graphql
type Mutation {
	create_dashboards_item(data: create_brio_dashboards_input!): brio_dashboards
}
```
...
```graphql
type Mutation {
	create_dashboards_items(data: [create_brio_dashboards_input!]!): [brio_dashboards]
}
```
...
```graphql
type Mutation {
	update_dashboards_item(id: ID!, data: update_brio_dashboards_input): brio_dashboards
}
```
...
```graphql
type Mutation {
	update_dashboards_items(ids: [ID!]!, data: update_brio_dashboards_input): [brio_dashboards]
}
```
