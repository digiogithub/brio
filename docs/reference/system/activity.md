---
description: REST and GraphQL API documentation on the Activity collection in Brio.
readTime: 4 min read
...
> All events within Brio are tracked and stored in the activities collection. This gives you full accountability
...
```graphql
type Query {
	activity_by_id(id: ID!): brio_activity
}
```
...
```graphql
type Mutation {
	create_comment(collection: String!, item: ID!, comment: String!): brio_activity
}
```
...
```graphql
type Mutation {
	update_comment(id: ID!, comment: String!): brio_activity
}
```
