---
description: REST and GraphQL API documentation on the Webhooks collection in Brio.
readTime: 5 min read
...
List all webhooks that exist in Brio.
...
```graphql
type Query {
	webhooks_by_id(id: ID!): brio_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_item(data: create_brio_webhooks_input!): brio_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_items(data: [create_brio_webhooks_input!]!): [brio_webhooks]
}
```
...
```graphql
type Mutation {
	update_webhooks_item(id: ID!, data: update_brio_webhooks_input!): brio_webhooks
}
```
...
```graphql
type Mutation {
	update_webhooks_items(ids: [ID!]!, data: update_brio_webhooks_input!): [brio_webhooks]
}
```
