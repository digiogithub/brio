---
description: API documentation on authentication in Brio.
readTime: 5 min read
...
There are two types of tokens that can be used to authenticate within Brio.
...
server-to-server communication. They are saved as plain-text within `brio_users.token`.
