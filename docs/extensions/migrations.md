---
description: A guide on how to setup your owns custom Migrations in Brio.
readTime: 2 min read
---

# Custom Migrations

> Brio allows adding custom migration files that run whenever the `brio database migrate:*` commands are
...
## Migrations and Brio schema

Migrations can be used to manage the contents of Brio collections (e.g. initial hydration). In order to do it, you
must ensure that the schema is up to date before running your migrations.

`brio database migrate:latest` runs the required Brio internal migrations and the migrations from `migrations`
...
of your Brio, because schema is not yet created when you run `migrate:latest`.
