---
description: A guide on how to scaffold your Brio Extension.
readTime: 5 min read
---

# Creating Extensions

## Scaffolding your Brio Extension

The easiest way to start developing extensions is to use the `create-brio-extension` utility:

```bash
npm init brio-extension
```
...
extension type and the language you chose. The most important one is `@brio/extensions-sdk`. This package includes a
...
The generated `package.json` file contains an additional `brio:extension` field with the following sub-fields:

- `type` — The type of the extension
- `path` — The path to the built extension
- `source` — The path to the source entrypoint
- `host` — A semver string that indicates with which versions of the Brio host, the extension is compatible with
...
Before your extension can be used by Brio, it has to be built. If you used the `create-brio-extension` utility
...
The generated `package.json` contains a script that calls the `brio-extension` CLI which is part of
`@brio/extensions-sdk`:

```json
{
	"scripts": {
		"build": "brio-extension build"
	}
}
```

If you prefer to scaffold your extension manually, you can use the `brio-extension` CLI binary directly. The
...
To make an extension available to all Brio users, you can publish the npm package created by
`@brio/extensions-sdk` to the npm registry. Make sure the name of the package follows the naming convention for
package extensions: `brio-extension-<extension-name>` or `@<scope>/brio-extension-<extension-name>`.
...
repository or any other means supported by npm. On startup, Brio will automatically load any package extension
installed into your Brio project folder.

To install an extension from the npm registry, simply use the npm CLI:

```bash
cd <brio-project-folder>
npm install <full-package-extension-name>
```

`<project-folder>` has to be replaced by the Brio project folder. `<full-package-extension-name>` should be replaced
with the full name of the package extension (e.g. `brio-extension-custom`).

### Local Extension

Local extensions are essentially the files generated by the `brio-extension build` command. They can be installed by
