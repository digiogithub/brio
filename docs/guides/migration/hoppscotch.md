---
description: Learn how to migrate your data model to a new Brio project using Hoppscotch.
tags: []
skill_level:
brio_version: 9.23.0
author_override:
...
You should have two Brio projects - this guide will refer to them as the "source" and the "target". Before starting,
make sure you have a static access token for both projects.

### Retrieve Data Model Snapshot From Source Project

![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.brio.io/docs/v9/cookbook/migration-hoppscotch/snapshot.webp)

1. Make sure `GET` is selected in the method dropdown.
2. In the URL field, enter your source Brio project URL followed by `/schema/snapshot`.
...
![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.brio.io/docs/v9/cookbook/migration-hoppscotch/diff.webp)
...
2. In the URL field, enter your target Brio project URL followed by `/schema/diff`.
...
![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.brio.io/docs/v9/cookbook/migration-hoppscotch/apply.webp)

1. Make sure `POST` is selected in the method dropdown.
2. In the URL field, enter your target Brio project URL followed by `/schema/apply`.
3. In the Parameters tab, set a query parameter called `access_token` with the access token for your source project.
...
The diff endpoint does not allow different Brio versions and database vendors by default. This is to avoid any
