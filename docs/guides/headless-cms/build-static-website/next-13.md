---
description: This guide shows you how build a website with Next 13 and Brio as a Headless CMS.
tags: []
skill_level:
brio_version: 9.21.4
author_override:
...
# Build a Website With Next 13 and the Brio JavaScript SDK
...
## Create Helper For @brio/sdk

To share a single instance of the Brio JavaScript SDK between multiple pages in this project, create a single helper
file that can be imported later. Create a new directory called `lib` and a new file called `brio.js` inside of it.
...
![A form named "Global" has two inputs - a title and a description, each filled with some text.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/next-global-config.webp)
...
```js
import brio from 'lib/brio';

async function getGlobals() {
	const { data } = await brio.items('global').readByQuery();
	return data;
}
...
browser. You should see data from your Brio Global collection in your page. Some additional files will be created by
...
[here's some sample data](https://github.com/nicobrio/examples/blob/main/website-next13/demo-data).
...
async function getPage(slug) {
	try {
		const page = await brio.items('pages').readOne(slug);
		return page;
	} catch (error) {
		notFound();
	}
...
async function getPosts() {
	const posts = await brio.items('posts').readByQuery({
...
![A page with a title of "Blog". On it is a list of three items - each with a title, author, and date. The title is a link.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-listing.webp)
...
async function getPost(slug) {
	try {
		const post = await brio.items('posts').readOne(slug, {
			fields: ['*.*'],
		});
		return post;
	} catch (error) {
...
![A blog post page shows an image, a title, and a number of paragraphs.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-single.webp)

## Add Navigation

While not strictly Brio-related, there are now several pages that aren't linked to each other. In `app/layout.tsx`,
...
Through this guide, you have set up a Next project, created a Brio helper, and used it to query data. You have used
...
[on GitHub](https://github.com/nicobrio/examples/blob/main/website-next13).
