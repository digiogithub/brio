---
description: This guide shows you how build a website with Nuxt 3 and Brio as a Headless CMS.
tags: []
skill_level:
brio_version: 9.21.4
author_override:
...
# Build a Website With Nuxt 3 and the Brio JavaScript SDK

> {{ $frontmatter.description }}

[Nuxt](https://nuxt.com/) is a popular JavaScript framework based on Vue.js. In this tutorial, you will learn how to
build a website using Brio as a CMS. You will store, retrieve, and use global metadata such as the site title,
create new pages dynamically based on Brio items, and build a blog.
...
- To sign up for a Brio Cloud account.
- Some knowledge of Vue.js and Nuxt.

Create a new Brio Cloud project - any tier and configuration is suitable for this tutorial.

Open your terminal and run the following commands to create a new Nuxt project and the Brio JavaScript SDK:
...
Ensure your Project URL is correct when initializing the Brio JavaScript SDK.

Inside of your `app.vue` entry file, add the following to the bottom to test that your plugin works:

```js
<script setup>const {$brio} = useNuxtApp() console.log($brio)</script>
```

Refresh your browser, and check the console. You should see the Brio instance logged, which means you have access to
all of the Brio JavaScript SDK methods by using the `useNuxtApp()` composable in any page or component.

Once you've confirmed this, remove the `<script>` from `app.vue` before continuing.

## Using Global Metadata and Settings

In your Brio project, navigate to Settings -> Data Model and create a new collection called `global`. Under the
...
![A form named "Global" has two inputs - a title and a description, each filled with some text.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/nuxt-global-config.webp)
...
<script setup>
  const { $brio } = useNuxtApp()
  const { data: global } = await useAsyncData('global', () => {
    return $brio.items('global').readByQuery()
  })
</script>
```

Refresh your browser. You should see data from your Brio Global collection in your page.

## Creating Pages With Brio
...
[here's some sample data](https://github.com/nicobrio/examples/blob/main/website-nuxt3/demo-data).
...
<script setup>
  ...
  const { data: page } = await useAsyncData('page', () => {
    return $brio.items('pages').readOne(route.params.slug)
  })
...
Go to `http://localhost:3000/about`, replacing `about` with any of your item slugs. Using the Brio JavaScript SDK,
...
In Roles & Permissions, give the Public role read access to the `authors`, `posts`, and `brio_files` collections.
...
<script setup>
  ...
  const { data: posts } = await useAsyncData('posts', () => {
    return $brio.items('posts').readByQuery({
...
![A page with a title of "Blog". On it is a list of three items - each with a title, author, and date. The title is a link.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/nuxt-blog-listing.webp)
...
```js
<template>
  <img :src="`${$brio.url}/assets/${post.image.filename_disk}?width=600`" alt="">
  <h1>{{post.title}}</h1>
...
<script setup>
  ...
  const { data: post } = await useAsyncData('post', () => {
    return $brio.items('posts').readOne(route.params.slug, {
      fields: ['*.*']
    })
  })
...
- In the `<img>` tag, `$brio.url` is the value provided when creating the Brio plugin.
- The `width` attribute demonstrates Brio' built-in image transformations.
...
- Because almost-all fields are used in this page, including those from the `image` relational field, the `fields`
  property when using the Brio JavaScript SDK can be set to `*.*`.

Click on any of the blog post links, and it will take you to a blog post page complete with a header image.

![A blog post page shows an image, a title, and a number of paragraphs.](https://cdn.brio.io/docs/v9/headless-cms/how-to-packet-20220222A/nuxt-blog-single.webp)

## Add Navigation

While not strictly Brio-related, there are now several pages that aren't linked to each other. In `app.vue`, above
...
Through this guide, you have set up a Nuxt project, created a Brio plugin, and used it to query data. You have used
...
[on GitHub](https://github.com/nicobrio/examples/blob/main/website-nuxt3).
