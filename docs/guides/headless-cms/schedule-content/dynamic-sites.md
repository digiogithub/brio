...
Scheduling content has fewer steps for a dynamic site. Since you are calling your Brio API at the time that a
...
### Check the Published Date When Calling the Brio API
...
Using the [Brio JavaScript SDK](/reference/sdk) (preferred)

```js
const articles = await brio.items('articles').readByQuery({
	filter: {
		_and: [
			{
				status: {
...
```js
const response = await fetch('https://yourbrio.com/items/articles?' + new URLSearchParams({
		filter: {
			_and: [
				{
					status: {
