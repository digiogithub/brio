import type { Field, Filter } from '@brio/types';
...
test('should append $thumbnail to fetched items when collection is brio_files', async () => {
	vi.mocked(useCollection).mockReturnValueOnce({ primaryKeyField: computed(() => null) } as any);

	const collection = ref('brio_files');
