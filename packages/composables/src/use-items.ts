import type { Item, Query } from '@brio/types';
import { getEndpoint, moveInArray } from '@brio/utils';
...
export function useItems(collection: Ref<string | null>, query: ComputedQuery): UsableItems {
	...
	async function getItems() {
		...
		try {
			 ...
			 * able to render out the brio_files collection (file library) using regular layouts
			 *
			 * Layouts expect the file to be a m2o of a `file` type, however, brio_files is the
			 ...
			 */
			if (collection.value === 'brio_files') {
