...
program.name('brio-extension').usage('[command] [options]');
program.version(getSdkVersion(), '-v, --version');

program
	.command('create')
	.arguments('<type> <name>')
	.description('Scaffold a new Brio extension')
	.option('-l, --language <language>', 'specify the language to use')
	.action(create);

program.command('add').description('Add entries to an existing Brio extension').action(add);

program
	.command('build')
	.description('Bundle a Brio extension to a single entrypoint')
...
program
	.command('link')
	.description('Creates a symlink to the extension in the Brio extensions folder')
	.argument('<path>', 'path to the extension folder of brio')
	.action(link);
