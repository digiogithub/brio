...
async function createPackageExtension({
	...
	const spinner = ora(chalk.bold('Scaffolding Brio extension...')).start();
...
function getPackageManifest(name: string, options: ExtensionOptions, deps: Record<string, string>) {
	const packageManifest: Record<string, any> = {
		name: EXTENSION_NAME_REGEX.test(name) ? name : `brio-extension-${name}`,
		description: 'Please enter a description for your extension',
		icon: 'extension',
		version: '1.0.0',
		keywords: ['brio', 'brio-extension', `brio-custom-${options.type}`],
		[EXTENSION_PKG_KEY]: options,
		scripts: {
			build: 'brio-extension build',
			dev: 'brio-extension build -w --no-minify',
			link: 'brio-extension link',
		},
