...
import { Collection as CollectionRaw, DeepPartial, Field } from '@brio/types';
import { Collection } from '@/types/collections';
import { getCollectionType } from '@brio/utils';
	...
	getters: {
		visibleCollections(): Collection[] {
			return this.collections
				.filter(({ collection }) => collection.startsWith('brio_') === false)
				.filter((collection) => collection.meta && collection.meta?.hidden !== true);
		},
		allCollections(): Collection[] {
			return this.collections.filter(({ collection }) => collection.startsWith('brio_') === false);
		},
		...
		crudSafeSystemCollections(): Collection[] {
			return orderBy(
				this.collections.filter((collection) => {
					return collection.collection.startsWith('brio_') === true;
				}),
				['collection'],
				['asc']
