...
import { Item, Panel } from '@brio/types';
import { getSimpleHash, toArray, applyOptionsData } from '@brio/utils';
...
export const useInsightsStore = defineStore('insightsStore', () => {
	...
	async function hydrate() {
		...
		if (
			permissionsStore.hasPermission('brio_dashboards', 'read') &&
			permissionsStore.hasPermission('brio_panels', 'read')
		) {
			try {
	...
	async function loadPanelData(
		...
		const gqlString = queryToGqlString(
			Array.from(queries.values())
				.filter(({ collection }) => {
					return collection.startsWith('brio_') === false;
				})
		...
		const systemGqlString = queryToGqlString(
			Array.from(queries.values())
				.filter(({ collection }) => {
					return collection.startsWith('brio_') === true;
				})
				.map(({ key, ...rest }) => ({
					key: `query_${key}`,
					...rest,
