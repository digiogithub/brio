<template>
	...
	<div v-else class="many-to-many">
					...
					>
						...
						<v-menu show-arrow placement="bottom-end">
							...
							<v-list>
								...
								<v-list-item
									clickable
									:download="element.brio_files_id.filename_download"
		...
		>
			<template #actions>
				<v-button
					v-if="currentlyEditing !== '+' && relationInfo.relatedCollection.collection === 'brio_files'"
					secondary
					rounded
					icon
...
import { getFieldsFromTemplate } from '@brio/utils';
...
const downloadName = computed(() => {
	if (relatedPrimaryKey.value === null || relationInfo.value?.relatedCollection.collection !== 'brio_files') return;
	const junctionField = relationInfo.value.junctionField.field;
	const relationPkField = relationInfo.value.relatedPrimaryKeyField.field;

	return displayItems.value.find((item) => get(item, [junctionField, relationPkField]))?.brio_files_id
		?.filename_download;
});
