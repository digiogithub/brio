...
import { Field, Query, Relation } from '@brio/types';
...
export function useItem(
	...
	function setItemValueToResponse(response: AxiosResponse) {
		if (
			(collection.value.startsWith('brio_') && collection.value !== 'brio_collections') ||
			(collection.value === 'brio_collections' && response.data.data.collection?.startsWith('brio_'))
		) {
