...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const boxSelectControl = new BoxSelectControl({
			...
			layers: ['__brio_polygons', '__brio_points', '__brio_lines'],
		});
		...
		function setupMap() {
			...
			map.on('load', () => {
				...
				const activeLayers = ['__brio_polygons', '__brio_points', '__brio_lines'];
				...
				map.on('click', '__brio_clusters', expandCluster);
				map.on('mousemove', '__brio_clusters', hoverCluster);
				map.on('mouseleave', '__brio_clusters', hoverCluster);
