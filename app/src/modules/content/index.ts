import { defineModule } from '@brio/utils';
...
import { Collection } from '@brio/types';
...
const checkForSystem: NavigationGuard = (to, from) => {
	...
	if (to.params.collection === 'brio_users') {
	...
	if (to.params.collection === 'brio_files') {
		if (to.params.primaryKey) {
	...
	if (to.params.collection === 'brio_activity') {
		if (to.params.primaryKey) {
	...
	if (to.params.collection === 'brio_webhooks') {
		if (to.params.primaryKey) {
	...
	if (to.params.collection === 'brio_presets') {
		if (to.params.primaryKey) {
