<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="brio_users" />
...
import { useLayout } from '@brio/composables';
import { Role } from '@brio/types';
import { mergeFilters } from '@brio/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('brio_users'));
...
const canInviteUsers = computed(() => {
	...
	const usersCreatePermission = permissionsStore.permissions.find(
		(permission) => permission.collection === 'brio_users' && permission.action === 'create'
	);

	const rolesReadPermission = permissionsStore.permissions.find(
		(permission) => permission.collection === 'brio_roles' && permission.action === 'read'
	);
...
function usePermissions() {
	const batchEditAllowed = computed(() => {
		...
		const updatePermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'update' && permission.collection === 'brio_users'
		);
	...
	const batchDeleteAllowed = computed(() => {
		...
		const deletePermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'delete' && permission.collection === 'brio_users'
		);
	...
	const createAllowed = computed(() => {
		...
		const createPermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'create' && permission.collection === 'brio_users'
		);
