...
import { InterfaceConfig, DisplayConfig, DeepPartial, Field, Relation, Collection, LocalType } from '@brio/types';
import { LOCAL_TYPES } from '@brio/constants';
		...
		async startEditing(collection: string, field: string, localType?: LocalType) {
			...
			if (field !== '+') {
				...
				if (['files', 'm2m', 'translations', 'm2a'].includes(this.localType)) {
					// These types rely on brio_relations fields being said, so meta should exist for these particular relations
