...
export async function up(knex: Knex): Promise<void> {
	const groupsInUse = await knex.select('id', 'group').from('brio_fields').whereNotNull('group');

	const groupIDs: number[] = uniq(groupsInUse.map(({ group }) => group));

	const groupFields = await knex.select('id', 'field').from('brio_fields').whereIn('id', groupIDs);
...
export async function down(knex: Knex): Promise<void> {
	...
	for (const { collection, group } of fieldsThatUseAGroup) {
		const { id } = await knex.select('id').from('brio_fields').where({ collection, field: group }).first();
	...
	await knex.schema.alterTable('brio_fields', (table) => {
		table.integer('group').references('id').inTable('brio_fields');
	});
