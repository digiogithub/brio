import type { OperationRaw } from '@brio/types';
import { z } from 'zod';
...
export const OperationsValidationSchema = z.discriminatedUnion('action', [
    z.strictObject({ action: z.literal('create'), data: OperationItemValidateSchema }),
    z.strictObject({ action: z.literal('read'), query: QueryValidateSchema.optional() }),
    z.strictObject({ action: z.literal('update'), data: OperationItemValidateSchema, key: z.string(), query: QueryValidateSchema.optional() }),
...
export const operations = defineTool<z.infer<typeof OperationsValidationSchema>>({
    name: 'operations',
    admin: true,
    description: 'CRUD operations for flow operations.',
    annotations: { title: 'Brio - Operations' },
