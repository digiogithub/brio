import type { Relation } from '@brio/types';
import { z } from 'zod';
...
const RelationsValidateSchema = z.discriminatedUnion('action', [
    z.object({ action: z.literal('create'), collection: z.string(), field: z.string().optional(), data: RelationItemValidateCreateSchema }),
    z.object({ action: z.literal('read'), collection: z.string().optional(), field: z.string().optional() }),
    z.object({ action: z.literal('update'), collection: z.string(), field: z.string(), data: RelationItemValidateUpdateSchema }),
    z.object({ action: z.literal('delete'), collection: z.string(), field: z.string() }),
]);
...
export const relations = defineTool<z.infer<typeof RelationsValidateSchema>>({
    name: 'relations',
    admin: true,
    description: 'CRUD operations for relations.',
    annotations: { title: 'Brio - Relations' },
    inputSchema: RelationsInputSchema,
    validateSchema: RelationsValidateSchema,
    async handler({ args, schema, accountability }) {
        const service = new RelationsService({ schema, accountability });
        ...
        if (args.action === 'delete') {
            await service.deleteOne(args.collection, args.field);
