import * as sharedExceptions from '@brio/exceptions';
import type {
	Accountability,
	ActionHandler,
	FilterHandler,
...
import { applyOptionsData, isValidJSON, parseJSON, toArray } from '@brio/utils';
		...
		if (flow.accountability !== null) {
			...
			if (flow.accountability === 'all') {
				...
				await revisionsService.createOne({
					...
					data: {
						steps: steps,
						data: redactLogs(omit(keyedData, '$accountability.permissions')), // Permissions is a ton of data, and is just a copy of what's in the brio_permissions table
					},
				});
			}
		}
