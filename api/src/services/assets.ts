import type { Range, Stat } from '@brio/storage';
...
export class AssetsService {
	...
	async getAsset(
		 ...
		 * with a wrong type. In case of brio_files where id is a uuid, we'll have to verify the
		...
		if (systemPublicKeys.includes(id) === false && this.accountability?.admin !== true) {
			await this.authorizationService.checkAccess('read', 'brio_files', id);
		}

		const file = (await this.knex.select('*').from('brio_files').where({ id }).first()) as File;
