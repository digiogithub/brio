...
import type { Accountability, Field, Permission, Relation, SchemaOverview, Type } from '@brio/types';
...
class OASSpecsService implements SpecificationSubService {
	...
	async generate() {
		...
		const spec: OpenAPIObject = {
			...
			servers: [
				{
					url: env['PUBLIC_URL'],
					description: 'Your current Brio instance.',
				},
			],
			paths,
		};
	...
	private async generateTags(collections: Collection[]): Promise<OpenAPIObject['tags']> {
		...
		for (const collection of collections) {
			const isSystem = collection.collection.startsWith('brio_');
	...
	private async generatePaths(permissions: Permission[], tags: OpenAPIObject['tags']): Promise<OpenAPIObject['paths']> {
		...
		for (const tag of tags) {
			const isSystem = 'x-collection' in tag === false || tag['x-collection'].startsWith('brio_');
