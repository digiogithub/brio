...
export class AuthenticationService {
	...
	async login(
		...
		const user = await this.knex
			...
			.from('brio_users as u')
			.leftJoin('brio_roles as r', 'u.role', 'r.id')
			.where('u.id', userId)
			.first();
		...
		if (allowedAttempts !== null) {
			...
			try {
				...
				await this.knex('brio_users').update({ status: 'suspended' }).where({ id: user.id });
		...
		await this.knex('brio_users').update({ last_access: new Date() }).where({ id: user.id });
	...
	async refresh(refreshToken: string): Promise<Record<string, any>> {
		...
		const record = await this.knex
			...
			.from('brio_sessions AS s')
			.leftJoin('brio_users AS u', 's.user', 'u.id')
			.leftJoin('brio_shares AS d', 's.share', 'd.id')
			.leftJoin('brio_roles AS r', (join) => {
		...
		if (record.user_id && record.user_status !== 'active') {
			await this.knex('brio_sessions').where({ token: refreshToken }).del();
		...
		if (record.user_id) {
			await this.knex('brio_users').update({ last_access: new Date() }).where({ id: record.user_id });
		}
	...
	async logout(refreshToken: string): Promise<void> {
		const record = await this.knex
			...
			.from('brio_sessions as s')
			.innerJoin('brio_users as u', 's.user', 'u.id')
			.where('s.token', refreshToken)
			.first();
		...
		if (record) {
			...
			await this.knex.delete().from('brio_sessions').where('token', refreshToken);
		}
	}
