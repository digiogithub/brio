...
describe('Integration Tests', () => {
	...
	beforeEach(() => {
		tracker.on.any('brio_roles').response({});

		tracker.on
			.select(/"brio_roles"."id" from "brio_roles" order by "brio_roles"."id" asc limit .*/)
			.response([]);
	});
				...
				describe('on an admin role', () => {
					...
					describe('with an array of user ids', () => {
						it('having an added user', async () => {
							...
							tracker.on.select('select "admin_access" from "brio_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "brio_users" where "role" = ?').responseOnce([{ id: userId1 }]);
						...
						it('having a removed user', async () => {
							...
							tracker.on
								.select('select "id" from "brio_users" where "role" = ?')
						...
						it('having a removed last user that is not the last admin of system', async () => {
							...
							tracker.on.select('select "admin_access" from "brio_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "brio_users" where "role" = ?').responseOnce([{ id: userId1 }]);
							tracker.on.select('select count(*) as "count" from "brio_users"').responseOnce({ count: 1 });
						...
						it('having a removed a last user that is the last admin of system', async () => {
							...
							tracker.on.select('select "admin_access" from "brio_roles"').responseOnce({ admin_access });
							tracker.on.select('select "id" from "brio_users" where "role" = ?').responseOnce([{ id: userId1 }]);
							tracker.on.select('select count(*) as "count" from "brio_users"').responseOnce({ count: 0 });
