...
describe('should fail on invalid snapshot schema', () => {
	...
	test('invalid schema', () => {
		const snapshot = { version: 1, brio: '9.22.4', collections: {} } as Snapshot;
...
describe('should require force option on version / vendor mismatch', () => {
	test('brio version mismatch', () => {
		const snapshot = { version: 1, brio: '9.22.3' } as Snapshot;

		expect(() => validateSnapshot(snapshot)).toThrowError(
			"Provided snapshot's brio version 9.22.3 does not match the current instance's version 9.22.4"
		);
	});

	test('db vendor mismatch', () => {
		const snapshot = { version: 1, brio: '9.22.4', vendor: 'postgres' } as Snapshot;
...
test('should allow bypass on version / vendor mismatch via force option ', () => {
	const snapshot = { version: 1, brio: '9.22.3', vendor: 'postgres' } as Snapshot;
