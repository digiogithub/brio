import type { DeepPartial } from '@brio/types';
...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'brio_users' },
	'author.role': { alias: 'ljnsv', collection: 'brio_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'brio_users' },
			'author.role': { alias: 'grenv', collection: 'brio_roles' },
		},
		relations: [
			{
				collection: 'articles',
	...
	expect(result.targetCollection).toBe('brio_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'brio_users' },
			'author.role': { alias: 'grenv', collection: 'brio_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'brio_roles' },
		},
		relations: [
			{
				collection: 'articles',
