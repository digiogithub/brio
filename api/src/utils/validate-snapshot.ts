import { TYPES } from '@brio/constants';
import Joi from 'joi';
...
import { version as currentBrioVersion } from './package.js';

const snapshotJoiSchema = Joi.object({
	version: Joi.number().valid(1).required(),
	brio: Joi.string().required(),
	vendor: Joi.string()
...
export function validateSnapshot(snapshot: Snapshot, force = false) {
	...
	if (snapshot.brio !== currentBrioVersion) {
		throw new InvalidPayloadException(
			`Provided snapshot's brio version ${snapshot.brio} does not match the current instance's version ${currentBrioVersion}. You can bypass this check by passing the "force" query parameter.`
		);
	}
